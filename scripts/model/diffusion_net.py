...
class DiffusionNet(Module):
    def __init__(self, net:TransformerModel, var_sched:VarianceSchedule, k=49, noise_loss=True, dist_loss=True, pos_loss=True):
        super().__init__()
        self.net = net
    ...
    def get_loss(self, x_0, context, std, mean, t=None):
        loss = {}
        ...
        if self.noise_loss:
            loss['noise'] = 1. * F.mse_loss(e_theta.contiguous().view(-1, point_dim), e_rand.contiguous().view(-1, point_dim), reduction='mean')

        # calc x_0 from x_t
        x_0_t = ((c0 * x_0 + c1 * e_rand) - c1 * e_theta) / c0

        # denormalize x_0 and x_0_t
        x_0_un = x_0.contiguous().view(batch_size, -1, 50, 2) * std.unsqueeze(dim=1) + mean.unsqueeze(dim=1)
        x_0_t_un = x_0_t.contiguous().view(batch_size, -1, 50, 2) * std.unsqueeze(dim=1) + mean.unsqueeze(dim=1)

        # distance loss
        if self.dist_loss:
            dist_0 = torch.cdist(x_0_un, x_0_un)
            dist_t = torch.cdist(x_0_t_un, x_0_t_un)

            dist_0_k, mask = torch.topk(dist_0, k=self.k+1, largest=False, dim=-1, sorted=True) # k+1 to get rid of point vs itself later
            dist_t_k = dist_t.gather(0, mask)
            loss['dist'] = 0.07 * F.mse_loss(dist_0_k[:, :, :, 1:], dist_t_k[:, :, :, 1:]) # get rid of point vs itself

        if self.pos_loss:
            # position loss
            loss['pos'] = 0.5 * F.mse_loss(x_0, x_0_t)
